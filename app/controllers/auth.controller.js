  ...
  await User.findOne({
    ...
    .then((data) => {
      if (data != null) {
        ...
        // Generated by Gemini CLI - If user exists but names are missing (common for manager-added users), update them
        ...
        if (needsUpdate) {
          logger.info(`Updating existing user ${user.id} with Google profile names`);
          User.update({ fName: user.fName, lName: user.lName }, { where: { id: user.id } })
            .catch(err => logger.error(`Error updating user names: ${err.message}`));
        }
      } else {
